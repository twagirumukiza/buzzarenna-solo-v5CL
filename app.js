...
(() => {
  ...
  async function tryResume(){
    ...
    const pid=state.uid;
    try{
      // Avec les règles sécurisées, un joueur doit être membre du salon avant de pouvoir le lire.
      // Une reconnexion tente donc d'abord de recréer son entrée joueur si elle a disparu.
      state.roomCode=saved.roomCode; state.playerId=pid; state.playerName=saved.playerName||'Joueur';
      try{
        await state.db.ref(`rooms/${saved.roomCode}/players/${pid}`).set({name:state.playerName,score:0,connected:true});
      }catch(_){ /* Déjà membre, partie démarrée, ou salon inexistant : on tente simplement la lecture. */ }
      const snap=await state.db.ref(`rooms/${saved.roomCode}`).once('value');
      if(!snap.exists()){ clearSession(); return false; }
      const room=snap.val();
      state.playerName=room.players?.[pid]?.name||state.playerName;
      if(!room.players || !room.players[pid]){ clearSession(); return false; }
  ...
  els.joinRoomBtn.addEventListener('click', async()=>{
    ...
    // On s'inscrit d'abord. Les règles Firebase vérifient elles-mêmes que le salon existe
    // et qu'il est encore en phase lobby. Une fois membre, la lecture du salon est autorisée.
    try{
      await state.db.ref(`rooms/${code}/players/${state.playerId}`).set({name,score:0,connected:true});
    }catch(err){
      console.error('Impossible de rejoindre le salon :',err);
      state.roomCode=null;
      return toast('Salon introuvable ou partie déjà commencée');
    }
    subscribeRoom(); saveSession();
    history.replaceState(null,'',`?room=${state.roomCode}`);
  });
