...
(() => {
  ...
  els.soloTimerRange.addEventListener('input',()=>els.soloTimerValue.textContent=`${els.soloTimerRange.value} s`);
  els.soloQuestionCountRange.addEventListener('input',()=>els.soloQuestionCountValue.textContent=els.soloQuestionCountRange.value);
  ...
  els.startSoloBtn.addEventListener('click', ()=>{
    const topics=$$('.soloTopicCheck').filter(c=>c.checked).map(c=>c.value); if(!topics.length) return toast('Choisissez au moins un thème');
    const name=els.soloName.value.trim()||'Joueur';
    const duration=+els.soloTimerRange.value, count=+els.soloQuestionCountRange.value;
    soloStart({topics,name,duration,count});
  });

  function soloStart(settings){
    const questions=buildQuestionSet(settings.topics,settings.count,true,[]);
    state.solo={ settings, questions, index:0, score:0, stats:{correct:0,answered:0,totalElapsedCorrect:0,streak:0,maxStreak:0,lightning:false} };
    state.timerFlags={};
    els.hostControls.classList.remove('hidden');
    els.endGameBtn.title='Terminer l’entraînement'; els.backToLobbyBtn.title='Recommencer l’entraînement';
    Presenter.say(`🎯 Entraînement de ${settings.name} : ${settings.count} questions sur ${topicsLabel(settings.topics)} !`,{holdMs:3400});
    soloRenderQuestion();
  }

  function soloRenderQuestion(){
    showView('gameView'); const solo=state.solo; const q=solo.questions[solo.index]; if(!q) return;
    solo.answered=null; solo.questionStartedAt=Date.now(); solo.questionEndAt=solo.questionStartedAt+solo.settings.duration*1000;
    stop(els.audioTimeEnd); play(els.audioAmbient,false); stop(els.audioTick); play(els.audioTick,false);
    els.roundLabel.textContent='ENTRAÎNEMENT';
    els.questionCounter.textContent=`Question ${solo.index+1} / ${solo.questions.length}`;
    ...
    els.myScore.textContent=solo.score;
    soloStartTimer();
    state.timerFlags={half:false,low:false};
    const seed=q.id||('solo'+solo.index);
    const delay=solo.index===0?2600:200;
    setTimeout(()=>{
      ...
      else { Presenter.say(Presenter.line('introNormal',seed,{i:solo.index+1,n:solo.questions.length}),{holdMs:2200}); }
    },delay);
  }

  function soloStartTimer(){
    clearInterval(state.currentTimer); const solo=state.solo; const duration=solo.settings.duration; const seed=(solo.questions[solo.index]?.id)||('solo'+solo.index);
    const tick=()=>{
      const remaining=Math.max(0,(solo.questionEndAt-Date.now())/1000); els.timerText.textContent=Math.ceil(remaining);
      ...
      if(remaining<=0){ clearInterval(state.currentTimer); stop(els.audioTick); play(els.audioTimeEnd); $$('.buzzer-btn').forEach(b=>b.disabled=true); soloFinalizeQuestion(null); }
    };
    tick(); state.currentTimer=setInterval(tick,200);
  }

  function soloHandleAnswer(choice, btn){
    const solo=state.solo; if(!solo || solo.finished || solo.answered) return;
    solo.answered={choice,submittedAt:Date.now()};
    $$('.buzzer-btn').forEach(b=>b.disabled=true); btn.classList.add('selected'); els.answerStatus.textContent='Réponse enregistrée'; play(els.audioBuzz);
    soloFinalizeQuestion(solo.answered);
  }

  function soloFinalizeQuestion(answer){
    const solo=state.solo; if(!solo || solo.finished) return;
    clearInterval(state.currentTimer); stop(els.audioTick);
    const q=solo.questions[solo.index]; const duration=solo.settings.duration*1000;
    const correct=!!answer && answer.choice===q.correct;
    const elapsed=answer? Math.max(0,answer.submittedAt-solo.questionStartedAt) : duration;
    const points=correct? Math.round((1000+500*Math.max(0,1-elapsed/duration))*q.multiplier) : 0;
    solo.score+=points;
    const st=solo.stats; st.answered+=1; if(correct) st.correct+=1;
    st.totalElapsedCorrect+= correct? elapsed:0;
    st.streak = correct? st.streak+1 : 0; st.maxStreak=Math.max(st.maxStreak,st.streak);
    st.lightning = st.lightning || (correct && elapsed<1000);
    solo.lastResult={correct,points,elapsed};
    if(st.streak===5) setTimeout(()=>Presenter.say(Presenter.line('badgeStreak','solo'+solo.index,{name:solo.settings.name}),{holdMs:2400}),3200);
    if(correct && elapsed<1000) setTimeout(()=>Presenter.say(Presenter.line('badgeLightning','solo'+solo.index+'l',{name:solo.settings.name}),{holdMs:2400}),3200);
    soloRenderResults();
  }

  function soloRenderResults(){
    const solo=state.solo; showView('resultsView'); const q=solo.questions[solo.index];
    els.correctAnswerLabel.textContent=`Bonne réponse : ${'ABCD'[q.correct]} — ${q.choices[q.correct]}`;
    const r=solo.lastResult;
    els.questionRanking.innerHTML=`<div class="rank-row ${r.correct?'correct':'wrong'}"><strong>${solo.index+1}</strong><span>${escapeHtml(solo.settings.name)} ${r.correct?'✓':'✕'}</span><strong>+${r.points}</strong></div>`;
    els.myScore.textContent=solo.score;
    els.nextProgress.style.width='0'; requestAnimationFrame(()=>{els.nextProgress.style.transition='width 3s linear';els.nextProgress.style.width='100%'});
    setTimeout(soloAdvance,3400);
  }

  function soloAdvance(){
    const solo=state.solo; if(!solo || solo.finished) return;
    if(solo.index<solo.questions.length-1){ solo.index+=1; soloRenderQuestion(); }
    else soloFinish();
  }

  function soloFinish(){
    const solo=state.solo, st=solo.stats;
    clearInterval(state.currentTimer); solo.finished=true;
    const avg = st.correct? (st.totalElapsedCorrect/st.correct/1000).toFixed(1) : '0.0';
    Presenter.say(Presenter.line('finalAnalysis','solo-final',{name:solo.settings.name,correct:st.correct,avg}),{holdMs:3800});
    setTimeout(()=>Presenter.say(Presenter.line('farewell','solo-farewell'),{holdMs:3400}),4000);
    els.championTrophy.textContent='🎯'; els.championEyebrow.textContent='ENTRAÎNEMENT TERMINÉ';
    els.championName.textContent=`Bravo ${solo.settings.name} !`;
    els.championStatsLine.textContent=`Score final : ${solo.score} points · ${st.correct}/${st.answered} bonnes réponses · temps moyen ${avg} s`;
    let badges=''; if(st.maxStreak>=5) badges+=' 🔥'; if(st.lightning) badges+=' ⚡'; if(st.correct>0 && st.correct===st.answered) badges+=' 🎯';
    els.finalRankingList.innerHTML=`<div class="rank-row"><strong>1</strong><span>${escapeHtml(solo.settings.name)}<span class="badges">${badges}</span></span><strong>${solo.score}</strong></div>`;
    els.newGameBtn.textContent='🔁 Rejouer'; els.newGameBtn.classList.remove('hidden'); els.waitingNewGame.classList.add('hidden');
  ...
  function syncHostControls(){
    ...
    if(show){ els.endGameBtn.title='Terminer la partie'; els.backToLobbyBtn.title='Retour au lobby'; }
  }
  ...
  els.buzzers.addEventListener('click',async e=>{
    ...
    if(state.solo){ soloHandleAnswer(+btn.dataset.choice, btn); return; }
  ...
  function renderStatsModal(){
    if(state.solo){
      const solo=state.solo, s=solo.stats;
      ...
      els.statsTableBody.innerHTML=`<div class="stats-row is-champion">
        <span data-label="Joueur">${escapeHtml(solo.settings.name)}</span>
        <span data-label="Score">${solo.score}</span>
  ...
  els.newGameBtn.addEventListener('click', ()=>{
    if(state.solo){ soloStart(state.solo.settings); return; }
    if(state.isHost) resetToLobby();
  });
  els.quitGameBtn.addEventListener('click',()=>{
    if(state.solo){ state.solo=null; els.hostControls.classList.add('hidden'); showView('homeView'); return; }
  ...
  els.endGameBtn.addEventListener('click', async()=>{
    if(state.solo){ if(!confirm('Terminer l’entraînement maintenant et voir vos statistiques ?')) return; soloFinish(); return; }
  ...
  els.backToLobbyBtn.addEventListener('click', async()=>{
    if(state.solo){ if(!confirm('Recommencer l’entraînement depuis le début avec les mêmes réglages ?')) return; soloStart(state.solo.settings); return; }
    if(!state.isHost) return;
